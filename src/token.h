...
int token_def_destroy(token_def_t *def) {
	if ( def->type == token_string ) {
		cstring_destroy((cstring_t*)def->payload);
		free(def->payload);
...
token_t *register_token_blank(token_store_t *store, const cstring_t *name) {
	token_t *token = malloc(sizeof(token_t));
	*(token_t**)vector_push_blank(&store->tokens) = token;
	*(int*)vector_push_blank(&store->match_buffer) = 0;
	*(token_t**)hashmap_push_blank_cs(&store->map, name) = token;

	cstring_init_copy(&token->name, name);
	token->def = NULL;
	return token;
}

token_t *register_token_string(token_store_t *store, const cstring_t *name, cstring_t *content) {
	token_t *token = register_token_blank(store, name);
	cstring_t *content_copy = malloc(sizeof(cstring_t));
	cstring_init_copy(content_copy, content);
	token_pushdef(token, token_def_create(token_string, content_copy));
	return token;
}

int match_token_c(token_store_t *store, char c) {
	for ( unsigned int i = 0; i < store->tokens.size; i++ ) {
		...
		if ( token->name.data[store->c_count] != c || token->def == NULL ) {
			*match = -1;
		}
...
token_t *get_token(token_store_t *store, const cstring_t *name) {
	token_t **token_ptr = (token_t**)hashmap_get_cs(&store->map, name);
	if ( token_ptr == NULL ) {
		return NULL;
	}
...
token_t *ensure_token(token_store_t *store, const cstring_t *name) {
...
token_t *get_token_match(token_store_t *store) {
	for ( unsigned int i = 0; i < store->tokens.size; i++ ) {
		...
		if ( *match == 0 ) {
			token_t *token = *(token_t**)vector_get(&store->tokens, i);
			if ( token->name.size == c_buffer.size ) {
				return token;
			}
		}
	}
